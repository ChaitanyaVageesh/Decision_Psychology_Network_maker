...
export async function POST(request: NextRequest) {
  try {
    ...
    // Main prompt for generating Bayesian network
    ...
    const { text: generatedNetwork } = await generateText({
      model: openai("gpt-4o"),
      prompt,
      temperature: 0.7,
      maxTokens: 2000,
    ...
    // === Step 2: Judge prompt to check for CPTs, states, edges completeness ===
    const judgePrompt = `
Your job is to strictly audit the following Bayesian Network specification. All of these must be fully and EXPLICITLY present, or else call out the deficiencies:
1. For every non-evidence node: a complete list of its possible states
2. For every node: a clearly presented Conditional Probability Table (CPT), with explicit states covering all possible cases
3. For every node: clear parent(s)-child(s) connection listing (edges)
4. No missing details for CPTs, states, or connections
5. If missing anything, enumerate: (a) which node, (b) missing states/CPT/edge, (c) direct suggestions to fix
6. If all is correct, say "VALID: All CPTs, states, and connections present and explicit for every node."
...
${generatedNetwork}
    ...
    // === Step 3: If not VALID, inject feedback for a fallback second attempt ===
    if (judgeVerdict.startsWith("VALID")) {
      return NextResponse.json({
        network: generatedNetwork.trim(),
        judgeVerdict: judgeVerdict.trim(),
      })
    } else {
      const feedbackFixPrompt = `
Please revise and regenerate the Bayesian Network as specified for the below situation and JSON. The following strict review was generated by an LLM judge. Carefully:
- Implement every specific suggestion for missing CPTs, states, or edges.
- Ensure every node has a CPT with all possible parent states.
- List every node's states explicitly.
- Clearly enumerate all parent-child connections.
- If any rationale was missing or unclear, expand as needed.

LLM Judge FEEDBACK:
${judgeVerdict.trim()}

Your improved Bayesian Network answer (fully specified and corrected), for:
SITUATION:
${situationDescription}
...
And adhere to the original order/structure demanded in the first modeling prompt.
`
      const { text: correctedNetwork } = await generateText({
        model: openai("gpt-4o"),
        prompt: feedbackFixPrompt,
        temperature: 0.65,
        maxTokens: 2200,
      })

      // Re-judge to see if fixes resolved issues
      const { text: finalJudgeVerdict } = await generateText({
        model: openai("gpt-4o"),
        prompt: judgePrompt.replace('${generatedNetwork}', correctedNetwork.trim()),
        temperature: 0.1,
        maxTokens: 600,
      })

      return NextResponse.json({
        network: correctedNetwork.trim(),
        judgeVerdict: finalJudgeVerdict.trim(),
        previousAttempt: {
          network: generatedNetwork.trim(),
          judgeVerdict: judgeVerdict.trim(),
        },
    ...
    return NextResponse.json(
      {
        ...
        fallbackSuggestion: "Double check your JSON data and situation description formatting.",
      },
      { status: 500 },
    )
  }
